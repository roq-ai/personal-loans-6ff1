...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getCashbackOffers();
    case 'POST':
      return createCashbackOffer();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getCashbackOffers() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const response = await prisma.cashback_offer
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'cashback_offer'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createCashbackOffer() {
    ...
    const data = await prisma.cashback_offer.create({
      data: body,
    });
